					...
					bool grouped)
{
	...
	for (i = pci->retc + ARG_OFFSET; i < (size_t)pci->argc; i++) {
		...
		} else if (bat_type == TYPE_str){// && has_output_str==0) {
                  	BATiter li;
			BUN p = 0, q = 0;
			bool can_mprotect_varheap = 1;
			str mprotect_retval;
...
//                        baseptr =  (str *)Tloc(input_bats[index], 0);
			...
			BATloop(input_bats[index], p, q)
			{
				...
				//if (can_mprotect_varheap) {
			        bat_data->data[j] = (char *)BUNtvar(li, p); //t;
				//} else {
		...
		} else if (bat_type == TYPE_str && 1 == 0){
                        BATiter li;
                        BUN p = 0, q = 0;
                        bool can_mprotect_varheap = 0;
                        str mprotect_retval;
                        GENERATE_BAT_INPUT_BASE(str);
                        bat_data->count = BATcount(input_bats[index]);
                        bat_data->data = bat_data->count == 0 ? NULL : GDKmalloc(sizeof(char *) * bat_data->count);
	...
	// call the actual jitted function
       /*if (!strcmp(funcname,"get_city_direct")){
         // printf("here\n");
        struct cudf_data_struct_str diverted = *((struct cudf_data_struct_str*)inputs[0]);
        struct cudf_data_struct_str* result = ((struct cudf_data_struct_str*)outputs[0]);
        result->initialize(result, diverted.count);
        msg = getcity(diverted.data, diverted.count,result->data);
        }
        else*/
	...
	for (i = 0; i < (size_t)pci->retc; i++) {
		...
		if (bat_type == TYPE_bit || bat_type == TYPE_bte ||
			...
			b = COLnew(input_bats[i]->hseqbase, TYPE_str, count, TRANSIENT);
			if (!b) {
				msg = createException(MAL, "cudf.eval", MAL_MALLOC_FAIL);
				goto wrapup;
			}
			if (bat_type == TYPE_date) {
              ...
              /*                  char*** pointers = malloc(count*sizeof(char**));
                                for (j = 0; j < count; j++) {
                                    pointers[j] = malloc(sizeof(char*));
                                    pointers[j] = source_base[j];
                                }
                                //b->T.vheap = pointers;
				//BATsetcount(b, count);
				b->batCount = count;
                                b->T.vheap->base = malloc(count*sizeof(char**)*sizeof(char*));
                                b->T.vheap->size = count*sizeof(char**)*sizeof(char*);
	                        memcpy(b->T.vheap->base, pointers, count); 
                                BATsetcount(b, count);
              //                  b = data;
                                //for (j = 0; j < count; j++) {
                              /*  int lsize = 0;
                                for (j = 0; j < count; j++) {
                                    lsize += strlen(source_base[j])+1;
                                }
                                b->T.vheap->size = lsize + count;
 				b->T.vheap->base = malloc(lsize + count); 
                                memcpy(b->T.vheap->base, (char **)data, lsize);
                                b->batCount = count;*/
                                //printf("calling multi\n\n");
 				BATsetcount(b, count);
                                for (j=0;j<count;j++)
                                    tfastins_nocheckVAR(b, j, ((char**)data)[j]);


//                                BUNappendmulti(b, source_base, count, false);
             			/*for (j = 0; j < count; j++) {
