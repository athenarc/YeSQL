...
class Cursor(object):
    ...
    def execute(self, statements, bindings=None, parse=True, localbindings=None):  # overload execute statement
        ...
        svts = sqltransform.transform(statements, list(multiset_functions.keys()), functions['vtable'],
                                      list(functions['row'].keys()), substitute=functions['row']['subst'])


        if sqltransform.inversion is False:
            self.__query = statements
            return self.executetrace(statements, bindings)
        else:
            sqlquery = rec_build_sql(svts[0], svts[1:])
            return self.executetrace(sqlquery, bindings)

            s = svts[0]
            try:
                if self.__vtables != []:
                    self.executetrace(''.join(['drop table ' + x + ';' for x in reversed(self.__vtables)]))
                    self.__vtables = []
                for i in svts[1]:
                    ...
                    createvirtualsql = VTCREATE + i[0] + ' as select * from ' + i[1] + "(" + i[2] +  ") on commit preserve rows;"
                    print(createvirtualsql)
                    try:
...
class Connection(pymonetdb.sql.connections.Connection):
    def cursor(self):
        ...
        return Cursor(pymonetdb.sql.connections.Connection.cursor(self))


    @echofunctionmember
    def close(self):
        pymonetdb.sql.connections.Connection.close(self)

def register(connection=None):
    global firstimport, oldexecdb

    if connection == None:
        connection = pymonetdb.connect(username=user, password=password, hostname=host, database=db, port=port,
                                       autocommit=True)
    ...
    ccur.executetrace("select distinct name,type from functions where type = 1 or type = 3 or type = 5;")
    try:
        while True:
            func = ccur.next()
            if func[0]!= 'insert':
...
def register_ops(module, connection):
    ...
    for f in module.__dict__:
        ...
        if hasattr(fobject, 'registered') and type(fobject.registered).__name__ == 'bool' and fobject.registered == True:
            ...
            if firstimport:
                if opname!=f:
                    raise YeSQLError("Extended SQLERROR: Function '"+module.__name__+'.'+f+"' uses uppercase characters. Functions should be lowercase")

                if opname.upper() in sqltransform.sqlparse.keywords.KEYWORDS:
                    raise YeSQLError("Extended SQLERROR: Function '"+module.__name__+'.'+opname+"' is a reserved SQL function")
            ...
            if type(fobject).__name__ == 'function':
                #if opexists(opname):
                #    raise YeSQLError("Extended SQLERROR: Row operator '"+module.__name__+'.'+opname+"' name collision with other operator")
